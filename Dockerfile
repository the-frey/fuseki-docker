...
RUN tar -xvzf /opt/fuseki/jena-fuseki-1.1.2.tar.gz -C /opt/fuseki/fuseki

ADD config.ttl /opt/fuseki/config/config.ttl

ADD run-fuseki /opt/fuseki/fuseki/run-fuseki

RUN chmod +x /opt/fuseki/fuseki/run-fuseki
...
CMD ["/opt/fuseki/fuseki/run-fuseki"]